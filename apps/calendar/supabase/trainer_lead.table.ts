...
export const reserveTrainer = async ({ trainerId, leadId }: { trainerId: string; leadId: number }) => {
  ...
  if (!deleteResponse.error) {
    const response = await supabaseClient
      .from(table)
      .upsert({ id_profiles: trainerId, id_lead: leadId, id_trainers: trainerId, accepted: true })
      .single();
    if (response.error) {
      throw response.error;
    }
...
export const setTrainerLeadEmail = async ({ id_trainer, leadId }: { id_trainer: string; leadId: string }) => {
  const response = await supabaseClient
    .from(table)
    .upsert({ id_profiles: id_trainer, id_lead: leadId, id_trainers: id_trainer, accepted: true })
    .single();
  if (response.error) {
    throw response.error;
  }
