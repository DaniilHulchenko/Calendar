import { ArchiveIcon, BanIcon, ExclamationIcon, ExternalLinkIcon, SaveIcon } from "@heroicons/react/outline";
...
import { LeadConflict, createLeadConflictName, terminateTrainer, useLeadsConflictQuery } from "supabase/lead_conflict";
...
export default function Conflicts() {
    ...
    const termianteTrainerMutation = useMutation({
        ...
        onSuccess: (updatedConflicted: any) => {
            // don`t refetch all data, just change in the cache
            conflicted.refetch();
    ...
    const refactoredConflicted = useMemo(() => {
        if (conflicted.data && trainersQuery.data) {
            {
                /*Utilize array methods like filter and map for data manipulation. */
            }
            return combineArrays(conflicted.data, trainersQuery.data).filter((conflicte) => !conflicte.isIgnore);
        }
    ...
    if (updateData) {
        ...
        return (
            <DefaultLayout icon={<BanIcon />} title={t("HR Conflicts")} description={t("Look at the conflicts of all leads")}>
                        ...
                        <div className="-mx-4 overflow-x-auto px-4 py-4 sm:-mx-8 sm:px-8">
                            <div className="inline-block min-w-full overflow-hidden rounded-lg shadow-md">
                                <table className="min-w-[605px] overflow-scroll leading-normal md:min-w-full">
                                    ...
                                    <tbody>
                                        {" "}
                                        {/*Use conditional rendering and ternary operators to simplify the code. */}
                                        ...
                                        {/* Split JSX into smaller components for better code organization. */}
                                        {refactoredConflicted?.map((conflictedInfo) => {
                                            if (conflictedInfo?.leads_id.length > 1) {
                                                /*Create small component for this row */
                                            }
                                            return (
                                                        ...
                                                        <div className="flex">
                                                            <div className="h-10 w-10 flex-shrink-0">
                                                                <ProfileAvatar
                                                                    ...
                                                                    small
                                                                    url={conflictedInfo?.trainer?.profile?.avatar?.url}
                                                                    className="h-full w-full rounded-full"
                                                                />
                                                            </div>
                                                            <div className="ml-3">
                                                                <p className="whitespace-no-wrap text-gray-900">
                                                                    {conflictedInfo?.trainer?.profile?.name || conflictedInfo?.trainer?.id}
                                                                </p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="row-g border-t border-gray-200 bg-white px-5 py-5 text-sm">
                                                        <div className="flex flex-col gap-[40px]">
                                                            ...
                                                            {conflictedInfo?.leads_id.length > 1 &&
                                                                conflictedInfo?.leads_id?.map((lead, index) => {
                                                                    return (
                                                                        <p key={index} className="whitespace-no-wrap text-gray-900">
                                                                            {JSON.parse(lead)?.leads?.customer_name || JSON.parse(lead)?.customer_name}
                                                                        </p>
                                                                    );
                                                                })}
                                                        </div>
                                                    ...
                                                    <td className="border-t border-gray-200 bg-white px-5 py-5 text-sm">
                                                        <div className="flex flex-col gap-[20px]">
                                                            ...
                                                            {conflictedInfo?.leads_id.length > 1 &&
                                                                conflictedInfo?.leads_id?.map((lead, index) => {
                                                                    return (
                                                                        <div key={index} className="border-b border-gray-200 last:border-b-[0px]">
                                                                            <p className="whitespace-no-wrap text-gray-900">
                                                                                {/*Extract repetitive logic into separate functions to improve code readability and maintainability. */}
                                                                                {t("Arrival At")}:{" "}
                                                                                {format(
                                                                                    new Date(JSON.parse(lead)?.leads?.arrival_at || JSON.parse(lead)?.arrival_at),
                                                                                    "dd.MM.yyyy",
                                                                                )}
                                                                            </p>
                                                                            {/*Extract repetitive logic into separate functions to improve code readability and maintainability. */}
                                                                            <p className="whitespace-no-wrap text-gray-600">
                                                                                {t("Departure")}: {/*Remove unnecessary checks and conditions */}
                                                                                {format(
                                                                                    new Date(JSON.parse(lead)?.leads?.departure || JSON.parse(lead)?.departure),
                                                                                    "dd.MM.yyyy",
                                                                                )}
                                                                            </p>
                                                                        </div>
                                                            ...
                                                            button={
                                                                <div
                                                                    onClick={(e) => {
                                                                        ...
                                                                        setCurrentTrainer(conflictedInfo.trainer_id);
                                                                                            ...
                                                                                            <div key={index} className="divide-y">
                                                                                                <label className="flex cursor-pointer items-start space-x-3 py-2">
                                                                                                    <input
                                                                                                        ...
                                                                                                        value={JSON.parse(lead)?.leads?.id || JSON.parse(lead)?.id}
                                                                                                    ...
                                                                                                    <div className="flex flex-col">
                                                                                                        ...
                                                                                                        <span className="leading-none text-gray-700">
                                                                                                            {/*Why customer_name exist in lead or lead.leads? */}
                                                                                                            {JSON.parse(lead)?.leads?.customer_name ||
                                                                            ...
                                                                            <div>
                                                                                <label className="flex cursor-pointer items-start space-x-3 py-2">
                                                                                    <input
                                                                                        type="radio"
                                                                                        name="lead"
                                                                                        onChange={(e) => setValue("ignore")}
                                                                                        value={"Ignore"}
                                                                                        className="h-5 w-5 rounded border-gray-300"
                                                                                    />
                                                                                    <div className="flex flex-col">
                                                                                        <span className="leading-none text-gray-700">{t("Ignore")}</span>
                                                                                    </div>
                                                                                </label>
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div className={"flex items-center justify-center gap-3 bg-gray-100 px-6 py-4"}>
                                                                        <Button
                                                                            onClick={() => {
                                                                                {
                                                                                    /*Extract repetitive logic into separate functions to improve code readability and maintainability. */
                                                                                }
                                                                                let filtered = conflictedInfo.leads_id.filter((lead) => {
                                                                                    lead = JSON.parse(lead);
                                                                                    if (value && +lead.id !== +value) return lead;
                                                                                });
                                                                                value &&
