...
const LeadExtra = ({
  ...
  const circlesNumber =
    lead?.required_trainers_amount &&
    lead.required_trainers_amount - leadTrainers.length;
  const currentSubSystem = lead?.subsystem;
  return (
    <Fragment>
      <div className="flex items-center p-1">
        <div className="grow truncate" title={location}>
          {!cellId ? (
          ...
          {!cellId ? (
            ...
            >
              <a className="flex items-center gap-1">
                ...
                {isManager && (
                  <span>
                    {joinedTrainers.data?.length &&
                    joinedTrainers.data?.length > 0
                      ? joinedTrainers.data?.length
                      : 0}
                  </span>
                )}
      ...
      <div className="relative flex items-center p-1">
                ...
                return (
                  ...
                  >
                    <div className=" h-[40px] w-auto">
                      <ProfileAvatar
                        url={trainers[index]?.profile?.avatar?.url}
                        small
                      />
                      ;
                    </div>
        ...
        >
          <div
            className={classNames(
              "bg-gradient-to- absolute top-0 -left-5 bottom-0 w-5",
              {
                "bg-white": currentSubSystem === "GmbH",
                "bg-emerald-100": currentSubSystem === "e.V.",
                "bg-blue-100": currentSubSystem === "Ausbildung",
      ...
      {cellId && cellId !== "new_cell" && (
        <LeadProgramBlockList
          leadId={cellId}
          programBlockNames={lead?.program_block_names}
        />
      )}
    </Fragment>
  );
