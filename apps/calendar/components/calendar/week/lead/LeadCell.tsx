...
LeadCellProps) => {
  ...
  useEffect(() => {
    ...
    if (
      cell.id === cellId &&
      open &&
      (cell.draft?.arrival_at !== prevDraftArrival.current ||
        cell.draft?.arrival_at !== lead.arrival_at)
    ) {
      updatePanelReference();
    }
  ...
  useEffect(() => {
    if (
      typeof router.query.cellId === "string" &&
      cellId &&
      cellId === Number.parseInt(router.query.cellId)
    ) {
  ...
  const hqStatusesOpen = [
    "Erstkontakt / Interesse",
    "Konkrete Anfrage",
    "Angebot",
    "Open",
  ];
  ...
  return (
    <motion.div
      ...
      className={classNames(
        "rounde transitio relative  mt-2 overflow-hidden rounded-md text-xs shadow-md", //w-screen
        {
          "rounded-l-lg": start,
          "rounded-r-lg": last,
        ...
        <div className="flex items-center p-1">
          <AvatarCircle
            ...
            className={classNames("m-1 flex-shrink-0", {
              "ring-yellow-500": hqStatusesOpen.includes(
                lead?.status || "NotSet"
              ),
              "ring-green-500": hqStatusesSuccessful.includes(
                lead?.status || "NotSet"
              ),
              "ring-red-500": hqStatusesUnsuccessful.includes(
                lead?.status || "NotSet"
              ),
            })}
            skeleton={!cellId}
